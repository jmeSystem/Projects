...
public class JmeRegistroVendaBean implements Serializable {
	...
	public JmeFormaPagamento getFormaPagamento() {
		...
		if(formaPagamento==null){
			formaPagamento = new JmeFormaPagamento();
		}
	...
	public void salvar() {
		try {
			...
			//problema se ouver colocar no if && pagamento.getValorPagamento() == vendaCadastro.getValorPagar()
			...
			if (pagamento.getNumeroParcelas() < 1) {
				...
				while (cont < pagamento.getNumeroParcelas()) {
					...
					parcela.setValorPagar(parcela.getValor());
	...
	public void confirmacao() {
		...
		if (vendaCadastro.getFormaPagamento().getIdFormaPagamento() != null) {
			...
			// Pesquisa a forma de pagamento a vista ou a prazo
			 formaPagamento = daoFormaPagamento
			...
			valorAcrescimo = formaPagamento.getAcrescimo(); 
