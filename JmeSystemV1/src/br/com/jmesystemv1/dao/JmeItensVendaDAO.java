...
public class JmeItensVendaDAO {
	...
	public JmeItensVenda buscarPorProduto(Integer codigo) {
		...
		try {
			Query consulta = sessao
					.getNamedQuery("JmeItensVenda.buscarPorProduto");
	...
	@SuppressWarnings("unchecked")
	public List<JmeItensVenda> buscar(JmeMovSaida filter){
		List<JmeItensVenda> movSaida= null;
		
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT AVG( movSaida.valorTotalItens ) from JmeItensVenda movSaida inner join JmeRegistroVendas registro ");
		
		if(filter.getDataInicial() !=null && filter.getDataFinal()!=null){
			sql.append("WHERE registro.dataVenda BETWEEN :dataInicial AND :dataFinal ");
		}
